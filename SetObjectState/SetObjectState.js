                    ...
                    case ownId + 'SetState':
                        ...
                        app.getObject(objId).then(function(object){
                            if (object.layout.qInfo.qType === 'filterpane') {
								...
								var tasks = [];
								for(var i=0; i < object.layout.qChildList.qItems.length; i++) {
									var qItem = object.layout.qChildList.qItems[i];
									console.log("Trying to get object " + qItem.qInfo.qId);
									tasks.push(app.getObject(qItem.qInfo.qId));
								}
								Promise.all(tasks).then((objects) => {
									var tasks = [];
									for(var i=0; i<objects.length; i++){
										console.log('Now patching object ' + objects[i].layout.qInfo.qId
													+ ' which is of type "' + objects[i].layout.qInfo.qType + '" to State "' + stateName + '"');
										var JSONpatch = JSON.parse('{"qPath":"/qListObjectDef/qStateName", "qOp":"add", "qValue":"\\"' + stateName + '\\"" }');
										tasks.push(objects[i].applyPatches([JSONpatch], false));
									}
                                ...
                                if(patchPath == '') {
                                    ...
                                    object.applyPatches([JSONpatch], false).then(() => {
										return app.doSave();
									}).then(() => {
										console.log('Object changed and app saved.');
				            			self.paint($element, layout);
                    ...
                    case ownId + 'AddState':
                        ...
                        if (stateName.length == 0) {
							...
							*/
							...
							app.addAlternateState(stateName).then(() => {
								return app.doSave();
							}).then(() => {
								console.log('State added and app saved.');
				                self.paint($element, layout);
